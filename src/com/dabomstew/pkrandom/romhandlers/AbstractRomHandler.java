...
public abstract class AbstractRomHandler implements RomHandler {
    ...
    public List<Move> getMoveSelectionPoolAtLevel(TrainerPokemon tp, boolean cyclicEvolutions) {
        ...
        double eggMoveProbability = 0.05;
    ...
    public void pickTrainerMovesets(Settings settings) {
        ...
        for (Trainer t: trainers) {
            for (TrainerPokemon tp: t.pokemon) {
                ...
                if (movesLeft <= 4) {
                    tp.move1 = distinctMoveList.get(0).number;
                    tp.move2 = movesLeft > 1 ? distinctMoveList.get(1).number : 0;
                    tp.move3 = movesLeft > 2 ? distinctMoveList.get(2).number : 0;
                    tp.move4 = movesLeft > 3 ? distinctMoveList.get(3).number : 0;
                    continue;
                }
                ...
                double atkSpatkRatio = pk.getAttackSpecialAttackRatio();
                ...
                if (pk.spatk > pk.attack && specialMoves.size() > 0) {
                    acceptedRatio = 1 - acceptedRatio;
                ...
                for (int i = 1; i <= 4; i++) {
                    ...
                    switch (i) {
                        case 1:
                            tp.move1 = move.number;
                            break;
                        case 2:
                            tp.move2 = move.number;
                            break;
                        case 3:
                            tp.move3 = move.number;
                            break;
                        default:
                            tp.move4 = move.number;
                            break;
                    }
                ...
                tp.move1 = pickedMoves.get(0).number;
                tp.move2 = movesPicked > 1 ? pickedMoves.get(1).number : 0;
                tp.move3 = movesPicked > 2 ? pickedMoves.get(2).number : 0;
                tp.move4 = movesPicked > 3 ? pickedMoves.get(3).number : 0;
            }
        }
    }
    ...
    private List<Move> trimMoveList(TrainerPokemon tp, List<Move> movesAtLevel, boolean doubleBattleMode) {
        if (movesAtLevel.size() <= 4) {
            tp.move1 = movesAtLevel.get(0).number;
            tp.move2 = movesAtLevel.size() > 1 ? movesAtLevel.get(1).number : 0;
            tp.move3 = movesAtLevel.size() > 2 ? movesAtLevel.get(2).number : 0;
            tp.move4 = movesAtLevel.size() > 3 ? movesAtLevel.get(3).number : 0;
